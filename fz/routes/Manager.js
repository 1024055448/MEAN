...
Operator.findByName(req.body.usernameAdd,function(err,doc){
	if(!err)
	{
		...
		{
			Operator.save(docs);
			res.render('Manager',{title:'添加成功',title2:'用户名：'+req.body.usernameAdd+'密码：'+req.body.passwordAdd});
		}
		else
		{
			res.render('Manager',{title:'该用户已经存在',title2:''});
		}
	}
	else
	{
...
router.post('/query',function(req,res,next) {
  ...
  Operator.findByName(req.body.username,function(err,doc){
       if(!err)
       {
        if(doc==null)
        {
        	res.render('Manager',{title: '查无此记录',title2:''});
        }
        else 
        {
        	res.render('Manager',{title: req.body.username,title2:req.body.password});
        }
...
Operator.findByName(req.body.usernameRemove,function(err,doc){
	if(!err)
	{
		if(!doc)
		{
			res.render('Manager',{title:'该用户不存在',title2:''});
		}
		else
		{
			Operator.remove(doc);
			res.render('Manager',{title:'删除成功',title2:'删除用户名：'+req.body.usernameRemove});
		}
	}
	else
	{
...
Operator.findByName(req.body.usernameModify,function(err,doc){
	if(!err)
	{
		...
		res.render('Manager',{title:'修改成功',title2:'修改后的用户名：'+req.body.usernameModifyF+'修改后的密码：'+req.body.passwordModifyF});
		}
		else
		{
			res.render('Manager',{title:'该用户不存在',title2:''});
